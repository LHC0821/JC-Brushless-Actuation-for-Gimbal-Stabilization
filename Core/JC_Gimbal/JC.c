#include "JC.h"
#include "uart.h"
#include "main.h"
#include <stdio.h>
#include <stdarg.h>

/* ================== 内部 CRC16 查询表与计算函数 ================== */
static const unsigned char auchCRCHi[256] = {
    
};

static const char auchCRCLo[256] = {
    
};

static uint16_t JC_CRC16(unsigned char *puchMsg, unsigned short usDataLen)
{
    unsigned char uchCRCHi = 0xFF; 
    unsigned char uchCRCLo = 0xFF; 
    unsigned uIndex ; 
    
    while (usDataLen--) 
    {
        uIndex = uchCRCLo ^ *puchMsg++; 
        uchCRCLo = uchCRCHi ^ auchCRCHi[uIndex];
        uchCRCHi = auchCRCLo[uIndex];
    }
    return (uchCRCHi << 8 | uchCRCLo);
}


/* ================================================================= */
/* USART 2 专 用                          */
/* ================================================================= */

void JC_Motor2_CloseLoop(void)
{
    uint8_t buf[8];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x06;  
    buf[2] = 0x00;  
    buf[3] = 0xA2;  
    buf[4] = 0x00;  
    buf[5] = 0x01;  
    
    crc = JC_CRC16(buf, 6);
    buf[6] = (uint8_t)crc;       
    buf[7] = (uint8_t)(crc >> 8); 
    
    USART2_SendArray(buf, 8);
}

//空闲环
void JC_Motor2_openLoop(void)
{
    uint8_t buf[8];
    uint16_t crc;
    
    buf[0] = 0x01;  // 电机 ID [cite: 175]
    buf[1] = 0x06;  // 写单个寄存器指令 [cite: 175]
    buf[2] = 0x00;  // 寄存器高字节 [cite: 175]
    buf[3] = 0xA0;  // 寄存器低字节 (空闲状态寄存器为0x00A0) [cite: 175]
    buf[4] = 0x00;  // 数据高字节 [cite: 175]
    buf[5] = 0x01;  // 数据低字节 (1代表执行) [cite: 175]
    
    crc = JC_CRC16(buf, 6);
    buf[6] = (uint8_t)crc;       
    buf[7] = (uint8_t)(crc >> 8); 
    
    USART2_SendArray(buf, 8);
}

void JC_Motor2_ReadVoltage(void)
{
    uint8_t buf[8];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x03;  
    buf[2] = 0x00;  
    buf[3] = 0x04;  
    buf[4] = 0x00;  
    buf[5] = 0x01;  
    
    crc = JC_CRC16(buf, 6);
    buf[6] = (uint8_t)crc;
    buf[7] = (uint8_t)(crc >> 8);
    
    USART2_SendArray(buf, 8);
}

void JC_Motor2_SetAbsAngle(int32_t angle)
{
    uint8_t buf[13];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x10;  
    buf[2] = 0x00;  
    buf[3] = 0x23;  
    buf[4] = 0x00;  
    buf[5] = 0x02;  
    buf[6] = 0x04;  
    buf[7] = (uint8_t)(angle >> 24); 
    buf[8] = (uint8_t)(angle >> 16); 
    buf[9] = (uint8_t)(angle >> 8);  
    buf[10] = (uint8_t)angle;        
    
    crc = JC_CRC16(buf, 11);
    buf[11] = (uint8_t)crc;
    buf[12] = (uint8_t)(crc >> 8);
    
    USART2_SendArray(buf, 13);
}

void JC_Motor2_SetRelAngle(int32_t angle)
{
    uint8_t buf[13];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x10;  
    buf[2] = 0x00;  
    buf[3] = 0x25;  
    buf[4] = 0x00;  
    buf[5] = 0x02;  
    buf[6] = 0x04;  
    buf[7] = (uint8_t)(angle >> 24); 
    buf[8] = (uint8_t)(angle >> 16); 
    buf[9] = (uint8_t)(angle >> 8);  
    buf[10] = (uint8_t)angle;  
    
    crc = JC_CRC16(buf, 11);
    buf[11] = (uint8_t)crc;
    buf[12] = (uint8_t)(crc >> 8);
    
    USART2_SendArray(buf, 13);
}

void JC_Motor2_PVTCommand(int32_t angle, uint16_t velocity, uint8_t torque_per)
{
    uint8_t buf[11];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x25;  
    buf[2] = (uint8_t)(angle >> 24); 
    buf[3] = (uint8_t)(angle >> 16); 
    buf[4] = (uint8_t)(angle >> 8);  
    buf[5] = (uint8_t)angle;  
    buf[6] = (uint8_t)(velocity >> 8); 
    buf[7] = (uint8_t)velocity;        
    buf[8] = torque_per;               
    
    crc = JC_CRC16(buf, 9);
    buf[9] = (uint8_t)crc;
    buf[10] = (uint8_t)(crc >> 8);
    
    USART2_SendArray(buf, 11);
}


/* ================================================================= */
/* USART 3 专 用                          */
/* ================================================================= */

void JC_Motor3_CloseLoop(void)
{
    uint8_t buf[8];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x06;  
    buf[2] = 0x00;  
    buf[3] = 0xA2;  
    buf[4] = 0x00;  
    buf[5] = 0x01;  
    
    crc = JC_CRC16(buf, 6);
    buf[6] = (uint8_t)crc;       
    buf[7] = (uint8_t)(crc >> 8); 
    
    USART3_SendArray(buf, 8);
}

void JC_Motor3_ReadVoltage(void)
{
    uint8_t buf[8];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x03;  
    buf[2] = 0x00;  
    buf[3] = 0x04;  
    buf[4] = 0x00;  
    buf[5] = 0x01;  
    
    crc = JC_CRC16(buf, 6);
    buf[6] = (uint8_t)crc;
    buf[7] = (uint8_t)(crc >> 8);
    
    USART3_SendArray(buf, 8);
}

void JC_Motor3_SetAbsAngle(int32_t angle)
{
    uint8_t buf[13];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x10;  
    buf[2] = 0x00;  
    buf[3] = 0x23;  
    buf[4] = 0x00;  
    buf[5] = 0x02;  
    buf[6] = 0x04;  
    buf[7] = (uint8_t)(angle >> 24); 
    buf[8] = (uint8_t)(angle >> 16); 
    buf[9] = (uint8_t)(angle >> 8);  
    buf[10] = (uint8_t)angle;        
    
    crc = JC_CRC16(buf, 11);
    buf[11] = (uint8_t)crc;
    buf[12] = (uint8_t)(crc >> 8);
    
    USART3_SendArray(buf, 13);
}

void JC_Motor3_SetRelAngle(int32_t angle)
{
    uint8_t buf[13];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x10;  
    buf[2] = 0x00;  
    buf[3] = 0x25;  
    buf[4] = 0x00;  
    buf[5] = 0x02;  
    buf[6] = 0x04;  
    buf[7] = (uint8_t)(angle >> 24); 
    buf[8] = (uint8_t)(angle >> 16); 
    buf[9] = (uint8_t)(angle >> 8);  
    buf[10] = (uint8_t)angle;  
    
    crc = JC_CRC16(buf, 11);
    buf[11] = (uint8_t)crc;
    buf[12] = (uint8_t)(crc >> 8);
    
    USART3_SendArray(buf, 13);
}

void JC_Motor3_PVTCommand(int32_t angle, uint16_t velocity, uint8_t torque_per)
{
    uint8_t buf[11];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x25;  
    buf[2] = (uint8_t)(angle >> 24); 
    buf[3] = (uint8_t)(angle >> 16); 
    buf[4] = (uint8_t)(angle >> 8);  
    buf[5] = (uint8_t)angle;  
    buf[6] = (uint8_t)(velocity >> 8); 
    buf[7] = (uint8_t)velocity;        
    buf[8] = torque_per;               
    
    crc = JC_CRC16(buf, 9);
    buf[9] = (uint8_t)crc;
    buf[10] = (uint8_t)(crc >> 8);
    
    USART3_SendArray(buf, 11);
}


void JC_Motor3_openLoop(void)
{
    uint8_t buf[8];
    uint16_t crc;
    
    buf[0] = 0x01;  
    buf[1] = 0x06;  
    buf[2] = 0x00;  
    buf[3] = 0xA0;  
    buf[4] = 0x00;  
    buf[5] = 0x01;  
    
    crc = JC_CRC16(buf, 6);
    buf[6] = (uint8_t)crc;       
    buf[7] = (uint8_t)(crc >> 8); 
    
    USART3_SendArray(buf, 8);
}

void JC_Motor_Stop(void){
    JC_Motor2_openLoop();
    JC_Motor3_openLoop();
}

void JC_Motor_SetZero(void){
    JC_Motor2_CloseLoop();
    JC_Motor3_CloseLoop();

    HAL_Delay(200); // 等待电机进入闭环状态

    JC_Motor2_SetAbsAngle(0.0f);
    JC_Motor3_SetAbsAngle(0.0f);
}